...
func adminRouterInit(r *gin.RouterGroup) {
	...
	stuAC := r.Group("/stuac", midwares.CheckSuppliesAdmin)
	{
		suppliesBorrow := stuAC.Group("/supplies-borrow")
		{
			...
			suppliesBorrow.POST("batch-supplies-check", suppliesController.CheckMoreRecordByAdmin)
			...
			suppliesBorrow.POST("batch-supplies-return", suppliesController.ReturnMoreRecordByAdmin)
